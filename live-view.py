...
if __name__ == '__main__':
    with GCSDevice(CONTROLLERNAME) as pidevice:
        ...
        while True:
            ...
            fft = work(frame[int(roimain[0][1]):int(roimain[0][1]+roimain[0][3]), int(roimain[0][0]):int(roimain[0][0]+roimain[0][2])])
        ...
        while frame_count < stacksize :
            ...
            for roim in roimain:
                ...
                avg = avgframe(lis)
                cv2.imshow("FFT of Bead 1",(zoom(np.asarray(work(avg), dtype=np.uint8))))
                cv2.waitKey(5)
            ...
            frame = cv2.flip(frame, 0)  # flip the frame vertically
            frame = cv2.flip(frame, 1)
            l3=zoom(work(frame[int(roiref[1]):int(roiref[1]+roiref[3]), int(roiref[0]):int(roiref[0]+roiref[2])]))
            l4= l3.tolist()
            stackref.append(FKA.FKA(l4))
        print(len(stack),len(stackref))
        # print(stackrange)
        with open('stack.pkl', 'wb') as f:
            load=[stack,stackref,roimain,roiref,stackrange]
            pickle.dump(load, f)
            f.close()
